		...
		while (foundMatch) {
			...
			if (match) {
				...
				if (!content.includes('{{#if')) {
					...
					const deepPattern = /\{\{#if ([^}]+)\}\}((?:(?!\{\{#if)[\s\S])*?)\{\{\/if\}\}/;
	...
	private parseConditionalParts(content: string): {
		...
		if (firstElseIfIndex < firstElseIndex) {
			...
			while ((match = elseIfRegex2.exec(remaining)) !== null) {
				...
				const nextElseMatch = /\{\{else\}\}/.exec(remaining.substring(startIndex));

				let endIndex: number;
				if (nextElseIfMatch && nextElseMatch) {
					endIndex =
						startIndex +
						Math.min(nextElseIfMatch.index, nextElseMatch.index);
			...
			const finalElseMatch = /\{\{else\}\}/.exec(remaining.substring(lastIndex));
			if (finalElseMatch) {
