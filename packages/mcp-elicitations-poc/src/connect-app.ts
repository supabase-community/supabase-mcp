...
export function createConnectApp(opts: {
  ...
  return {
    async fetch(req) {
      ...
      if (!principal) return response("A valid mock dashboard session is required.", 401);
      ...
      if (req.method === "GET") {
        ...
        secret = typeof form.get("secret") === "string" ? String(form.get("secret")) : null;
      } else {
      ...
      if (!interaction) return response("Interaction not found or expired.", 404);
      ...
      return response("Your API key is stored. You can return to your client.", 200);
    },
  };
}
