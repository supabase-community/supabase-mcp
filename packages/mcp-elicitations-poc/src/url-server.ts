...
function result(structuredContent: Record<string, unknown>, text: string, isError = false) {
  return {
...
export function createUrlPoc(opts: UrlPocOptions = {}): UrlPoc {
  ...
  const mintState = (ctx: ServerContext, sub: string, name: string, interactionId: string) =>
    codec.mint({
      v: 1,
      sub,
      tool: "store_api_key",
