...
describe('create_project MRTR happy path', () => {
  ...
  it.each([
    ...
    'reissues input_required when inputResponses are %s',
    async (_, responses) => {
      const poc = createPoc();
  ...
  it.each([
    ['missing confirm', {}],
    ['wrong-typed confirm', { confirm: 'yes' }],
  ])(
    'reissues input_required when accepted content has %s',
    async (_, content) => {
      const poc = createPoc();
      const first = await rawToolCall({
        poc,
      ...
      const completed = await rawToolCall({
        poc,
        declareElicitation: true,
        args,
        inputResponses: {
        ...
        requestState: retry.body.result.requestState as string,
      });

      expect(completed.status).toBe(200);
      expect(completed.body.error).toBeUndefined();
      expect(structured(completed.body.result).status).toBe('created');
      expect(poc.registry.list()).toHaveLength(1);
      expect(poc.registry.countByName(args.name)).toBe(1);
    }
  );
