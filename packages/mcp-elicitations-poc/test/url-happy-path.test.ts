...
import { openConnectPage, rawUrlToolCall, submitSecret } from "./url-harness.js";
...
describe("URL-mode elicitation happy path", () => {
  it("waits for the out-of-band interaction, then returns only secret metadata", async () => {
    const poc = createUrlPoc({ stateKey: "url-happy-path-state-key-at-least-32-bytes" });
    ...
    expect(firstRequest).toMatchObject({ mode: "url", url: expect.any(String) });
    ...
    expect((await openConnectPage({ poc, url: firstRequest.url })).status).toBe(401);
    expect((await openConnectPage({ poc, url: firstRequest.url, session: "user-bob" })).status).toBe(403);
    expect((await openConnectPage({ poc, url: firstRequest.url, session: "user-alice" })).status).toBe(200);
    expect((await submitSecret({ poc, interactionId: id, secret: "sk-test-1234", session: "user-alice" })).status).toBe(200);
  ...
  for (const action of ["decline", "cancel"] as const) {
    it(`returns ${action} as a normal, distinct result`, async () => {
      ...
      expect(final.body.result.structuredContent).toEqual({ status: `${action}${action === "cancel" ? "led" : "d"}` });
