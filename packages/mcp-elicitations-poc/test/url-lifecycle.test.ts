...
describe("URL-mode lifecycle", () => {
  describe("accept before completion", () => {
    it("reissues fresh state for the same pending interaction", async () => {
      ...
      expect(waiting.body.result.requestState).not.toBe(first.body.result.requestState);
    ...
    for (const [label, responses] of [
      ...
      it(`reissues for ${label}`, async () => {
        const poc = createUrlPoc();
        const first = await start(poc);
        const waiting = await retry(poc, first.body.result.requestState, responses);
  ...
  describe("expiry", () => {
    it("returns 404 when the connect page interaction has expired", async () => {
      ...
      expect(page).toEqual({ status: 404, body: "Interaction not found or expired." });
      expect(poc.secrets.get("user-alice", "github")).toBeUndefined();
    });
    ...
    it("rejects an accepted retry after interaction expiry", async () => {
      ...
      expect(expired.body.result.content[0].text).toBe("The interaction is missing or expired.");
      expect(expired.body.result.structuredContent).toEqual({ status: "error" });
      expect(poc.secrets.get("user-alice", "github")).toBeUndefined();
    });
    ...
    it("can complete a fresh flow after an expiry rejection", async () => {
      ...
      now += 1_001;
      expect((await retry(poc, expiredFlow.body.result.requestState)).body.result.isError).toBe(true);
  ...
  describe("one-time redemption", () => {
    it("rejects an identical completing retry without a second secret write", async () => {
      ...
      const complete = await rawUrlToolCall({ poc, args: { name: "github" }, ...completing });
      const stored = poc.secrets.get("user-alice", "github");
      const replay = await rawUrlToolCall({ poc, args: { name: "github" }, ...completing });
      ...
      expect(stored?.ref).toBe(complete.body.result.structuredContent.secret_ref);
    });
  ...
  describe("URL capability gating", () => {
    for (const capability of ["form-only", "none"] as const) {
      it(`returns unsupported_client without a URL request for ${capability}`, async () => {
        const poc = createUrlPoc();
        const connection = await createUrlTestClient({ poc, capabilities: capability });
        try {
          ...
          expect(result.structuredContent).toEqual({
            ...
            message: "A browser-capable client that declares URL elicitation is required.",
          });
          expect(JSON.stringify(connection.wire)).not.toMatch(/inputRequests|\"mode\":\"url\"/);
          expect(poc.secrets.get("user-alice", "github")).toBeUndefined();
        } finally {
    ...
    it("sends a URL request without a requested schema to a URL-capable client", async () => {
      ...
      expect(request(first)).toMatchObject({ mode: "url", url: expect.any(String) });
