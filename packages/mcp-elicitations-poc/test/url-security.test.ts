...
describe('URL-mode security', () => {
  describe('secret isolation from the MCP wire', () => {
    it('keeps the secret on the connect channel and returns metadata only', async () => {
      const poc = createUrlPoc({
        stateKey: 'url-security-wire-state-key-at-least-32-bytes',
      });
      let connectUrl = '';
      const testClient = await createUrlTestClient({
        ...
        onUrl: (async (url: string) => {
          connectUrl = url;
          const opened = await openConnectPage({
            poc,
            url,
            session: 'user-alice',
          });
      ...
      try {
        ...
        expect(
          new Set(testClient.wire.map((frame) => frame.direction))
        ).toEqual(new Set(['request', 'response']));
  ...
  describe('phishing binding', () => {
    it("rejects missing and mismatched sessions without wedging Alice's flow", async () => {
      const poc = createUrlPoc({
        stateKey: 'url-security-phishing-state-key-at-least-32-bytes',
      });
      const first = await rawUrlToolCall({
        poc,
        bearer: 'user-alice',
        args: { name: 'github' },
      });
      ...
      const nobody = await openConnectPage({
        poc,
        url,
        session: 'user-nobody',
      });
      ...
      expect(complete.body.result.structuredContent).not.toHaveProperty(
        'last4'
      );
      expect(poc.secrets.get('user-alice', 'github')).toEqual({
  ...
  describe('cross-principal state redemption', () => {
    it("rejects Mallory's retry and leaves Alice's stored secret untouched", async () => {
      const poc = createUrlPoc({
        stateKey: 'url-security-redemption-state-key-at-least-32-bytes',
      });
      const first = await rawUrlToolCall({
        poc,
        bearer: 'user-alice',
      ...
      expect(JSON.stringify(mallory.body)).not.toMatch(
        /secret_ref|sk-live-SENTINEL/
      );
