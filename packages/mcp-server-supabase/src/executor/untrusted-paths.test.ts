...
describe('requiresBoundary', () => {
  test('POST to database/query returns true', () => {
    expect(requiresBoundary('POST', '/v1/projects/abc123/database/query')).toBe(true);
  });
  test('POST to database/query/read-only returns true', () => {
    expect(requiresBoundary('POST', '/v1/projects/abc123/database/query/read-only')).toBe(true);
  });
  test('GET to logs endpoint returns true', () => {
    expect(requiresBoundary('GET', '/v1/projects/abc123/analytics/endpoints/logs.all')).toBe(true);
  });
  test('GET to edge function body returns true', () => {
    expect(requiresBoundary('GET', '/v1/projects/abc123/functions/my-func/body')).toBe(true);
  });
  ...
  test('GET to database/query (wrong method) returns false', () => {
    expect(requiresBoundary('GET', '/v1/projects/abc123/database/query')).toBe(false);
  });
  ...
  test('strips query string before matching', () => {
    expect(requiresBoundary('POST', '/v1/projects/abc123/database/query?foo=bar')).toBe(true);
  });
