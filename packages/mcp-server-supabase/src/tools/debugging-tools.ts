  ...
  return {
    get_logs: injectableTool({
      ...
      parameters: z.object({
        ...
        time_window: z
          .enum(['1min', '5min', '15min', '1hour', '24hour'])
          .default('1hour')
          .describe('Time window for logs (1min=last minute, 5min=last 5 minutes, 24hour=last 24 hours, etc.)'),
        log_level_filter: z
      ...
      execute: async ({
        ...
        // Get logs from API using service-based approach
