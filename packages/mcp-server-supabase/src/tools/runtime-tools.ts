...
export function getRuntimeTools() {
  return {
    toggle_read_only_mode: tool({
      description: 'Toggle between read-only and write modes for database operations. Read-only mode prevents all database modifications, while write mode allows full database access. Claude CLI users receive interactive confirmation prompts.',
      annotations: {
      ...
      parameters: z.object({
        confirm_write_mode: z.boolean().optional()
          .describe('Set to true to confirm switching to write mode (required when enabling write operations)')
      }),
      execute: async (args) => {
    ...
    get_runtime_mode_status: tool({
      description: 'Get the current runtime mode status, including read-only state, security information, and Claude CLI specific guidance.',
      annotations: {
      ...
      execute: async () => {
        ...
        return {
          ...
          next_steps: currentMode.readOnly ?
            ['Use database query tools safely', 'Toggle to write mode if modifications needed'] :
            ['Use caution with database modifications', 'Consider toggling back to read-only when done'],
    ...
    set_read_only_mode: tool({
      description: 'Explicitly set read-only mode to enabled or disabled. Use toggle_read_only_mode for interactive switching.',
      annotations: {
      ...
      parameters: z.object({
        read_only: z.boolean()
          .describe('True to enable read-only mode, false to enable write mode'),
        confirm_write_mode: z.boolean().optional()
          .describe('Required confirmation when enabling write mode (setting read_only to false)')
      }),
      execute: async (args) => {
        const modeManager = getModeManager();
        ...
        if (!read_only) {
          ...
          if (validation.confirmationRequired && !confirm_write_mode) {
            const message = '🔓 Claude CLI: Enabling write mode requires confirmation.\n\n⚠️  Write mode allows potentially destructive database operations.\n\nTo proceed, call this tool again with confirm_write_mode: true';
    ...
    validate_mode_change: tool({
      description: 'Check if a mode change is allowed and what confirmations are required. Useful for understanding requirements before attempting mode changes.',
      annotations: {
      ...
      parameters: z.object({
        target_mode: z.enum(['read-only', 'write'])
      ...
      execute: async (args) => {
        ...
        if (validation.confirmationRequired) {
          response.how_to_confirm = {
            tool: target_mode === 'write' ? 'toggle_read_only_mode' : 'set_read_only_mode',
    ...
    switch_project: tool({
      description: 'Switch to a different Supabase project. Claude CLI users get an interactive project selection interface when multiple projects are available.',
      annotations: {
      ...
      parameters: z.object({
        project_identifier: z.string().optional()
          .describe('Project ID or name to switch to. If not provided, lists available projects for selection.')
      }),
      execute: async (args) => {
          ...
          if (!project_identifier) {
            ...
            if (projectList.projects.length === 0) {
              return {
                ...
                claude_cli_message: '📋 Claude CLI: No projects found. Create a project at https://supabase.com/dashboard'
              };
            }
            ...
            return {
              success: true,
              message: 'Available projects listed. Specify project_identifier to switch.',
              ...
              claude_cli_message: 'Select a project by calling this tool again with project_identifier (ID or name).'
            };
          }

          // Switch to specified project
          const result: ProjectSwitchResult = await switchProjectInteractiveClaudeCLI(project_identifier);
    ...
    get_current_project: tool({
      description: 'Get information about the currently selected Supabase project, including project details and switching guidance.',
      annotations: {
    ...
    list_projects: tool({
      description: 'List all available Supabase projects with detailed information. Claude CLI users get a formatted display optimized for project selection.',
      annotations: {
        title: 'List projects',
        readOnlyHint: true,
        destructiveHint: false,
      ...
      parameters: z.object({
        refresh: z.boolean().optional()
          .describe('Force refresh of project list from API (default: false, uses 5-minute cache)')
      }),
      execute: async (args) => {
        ...
        try {
          ...
          if (projectList.hasMultipleProjects) {
            response.claude_cli_message = 'Use switch_project with project_identifier to change active project.';
          }
