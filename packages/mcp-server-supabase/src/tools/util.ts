...
>({
  ...
  return tool({
    ...
    outputSchema,
    hidden,
    // A policy decides on the same arguments the tool executes with, so the
    // injected values are merged in before it sees them. Without this, a
    // project-scoped server would ask a policy to reason about a project it
