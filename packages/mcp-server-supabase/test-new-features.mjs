...
import { StreamTransport } from '../mcp-utils/dist/index.js';
import { createSupabaseMcpServer } from './dist/index.js';
...
async function setupTestClient() {
  ...
  // Note: Not setting projectId to allow account tools to be available
...
async function testNewFeatures() {
  ...
  try {
    ...
    const snippetTools = ['list_sql_snippets', 'get_sql_snippet'];
    ...
    // Test list_sql_snippets execution
    if (toolMap['list_sql_snippets']) {
      try {
        const result = await client.callTool({
        ...
        logTest(
          'list_sql_snippets execution',
          success,
          `Found snippets in response`
        );
        if (success) snippetTestsPassed++;
      } catch (err) {
        logTest('list_sql_snippets execution', false, err.message);
      }
    }
